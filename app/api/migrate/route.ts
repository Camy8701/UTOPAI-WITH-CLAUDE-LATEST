      ...
      cookies: {
        ...
        set(name: string, value: string, options: any) {
          try {
            ...
            // In API routes, we can't always set cookies, so we ignore this error
          }
        },
        remove(name: string, options: any) {
          try {
            ...
            // In API routes, we can't always remove cookies, so we ignore this error
          }
        },
      },
    }
...
- **Business:** Draft emails, create presentations, analyze data, develop strategies
- **Education:** Explain concepts, create lesson plans, generate quiz questions
- **Content Creation:** Blog posts, social media content, video scripts
- **Technical Support:** Troubleshoot problems, write documentation, explain technical concepts

**Best Practices for Using ChatGPT:**

1. Be specific in your requests for better results
2. Ask follow-up questions to dive deeper into topics
3. Use it as a starting point, then refine and personalize the output
4. Fact-check important information, especially for recent events
5. Experiment with different prompt styles to find what works best for your needs
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔍 Migration attempt started...')
    ...
    if (authError || !user) {
      ...
      return NextResponse.json(
        { 
          ...
          debug: 'Make sure you are logged in and try again'
        },
        { status: 401 }
      )
    }
    ...
    console.log(`👤 Using author ID: ${user.id} (${user.email})`)
    ...
    for (const post of allPosts) {
      ...
      if (error) {
        ...
        return NextResponse.json(
          { error: `Failed to create post: ${post.title}`, details: error },
          { status: 500 }
        )
      } else {
